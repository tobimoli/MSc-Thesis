...
#%% select area
Lo1, Lo2, La1, La2 = 25, 25.40,39.75,40.05
#Lo1, Lo2, La1, La2 = 24.0,26.20,38.40,42.70
...
#%% Select a subset of in-situ observations to speed up the process
import random
random.seed(10)
randomlist = random.sample(range( len(x_situ)), 1000)
...
sigma_n = 1
sigma_f = 1
l = np.array([100, 100, 100, 1, 1, 1, 1])

start = time.time()
for i in range(1):
    kern = GPy.kern.sde_Matern32(7, variance=sigma_f, lengthscale=l, ARD = True)
...
plotgrid(prediction[0], 'Concentration Chlorophyll-a using GPR', plot_insitu=False, var_insitu = 'chl')
